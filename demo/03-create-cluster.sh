...
assign_roles_to_managed_identities() {
  ...
  # Get integration subnet ID
  local int_vnet_id
  int_vnet_id=$(az network vnet subnet show \
    --resource-group "${CUSTOMER_RG_NAME}" \
  ...
  assign_role "${service_mi_principal_id}" "${hcp_service_mi_role}" "${int_vnet_id}" "HCP Service MI role on Integration VNet"
...
main() {
  ...
  INT_SUBNET_ID=$(az network vnet subnet show --resource-group "${CUSTOMER_RG_NAME}" --vnet-name "${CUSTOMER_VNET_NAME}" --name "${CUSTOMER_INT_VNET_SUBNET1}" --query id --output tsv)
  ...
  jq \
    ...
    --arg integration_subnet_id "$INT_SUBNET_ID" \
    --arg nsg_id "$NSG_ID" \
    ...
    '
      ...
      .properties.platform.vnetIntegrationSubnetId = $integration_subnet_id |
